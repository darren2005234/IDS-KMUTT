...
def classify_batch(request):
    """POST /api/classify_batch/  body = {"flows": [ {..features.., src_ip,...}, ... ]}"""
    import json as json_lib
    ...
    if not flows:
        return Response({'created': 0, 'alerts': 0})

    # Vectorised ML prediction for the whole batch
    ml_results = predict_batch(flows)

    objs = []
    alerts = 0
    for flow, ml in zip(flows, ml_results):
        snort_alert = bool(flow.get('snort_alert', False))
        snort_sid   = flow.get('snort_sid')
        ml_pred     = ml['ml_prediction']
        ml_conf     = ml['ml_confidence']

        # Same fusion rules as fuse(), inlined for bulk insert
        if ml_pred == 1 and snort_alert:
            decision, source_tag = 'THREAT', 'BOTH'
        elif ml_pred == 1 and not snort_alert and ml_conf >= 0.50:
            decision, source_tag = 'ALERT', 'ML_ONLY'
        elif ml_pred == 1 and not snort_alert and ml_conf < 0.50:
            decision, source_tag = 'BENIGN', 'NONE'
        elif ml_pred == 0 and snort_alert:
            decision, source_tag = 'THREAT', 'SNORT_ONLY'
        else:
            decision, source_tag = 'BENIGN', 'NONE'

        if decision in ('THREAT', 'ALERT'):
            alerts += 1
        ...
        objs.append(Alert(
            ...
            ml_prediction=ml_pred, ml_confidence=ml_conf,
            ml_model_used=ml['ml_model_used'], attack_type=ml['attack_type'],
            detection_mode=ml['detection_mode'],
            snort_alert=snort_alert, snort_sid=snort_sid,
            decision=decision, source_tag=source_tag,
        ))

    # Single DB round-trip instead of N inserts
    Alert.objects.bulk_create(objs, batch_size=1000)
    return Response({'created': len(objs), 'alerts': alerts},
...
def fusion_engine_page(request):
    ...
    # Vote distribution
    ml_attack_qs = Alert.objects.filter(source_tag__in=['ML_ONLY', 'BOTH'])
    votes_1 = ml_attack_qs.exclude(ml_model_used__contains='+').count()
    votes_2 = ml_attack_qs.filter(ml_model_used__in=[
        'RandomForest+XGBoost', 'RandomForest+LSTM', 'XGBoost+LSTM'
    ]).count()
    votes_3 = ml_attack_qs.filter(ml_model_used='RandomForest+XGBoost+LSTM').count()

    # Model detection rates
    rf_detections   = ml_attack_qs.filter(ml_model_used__contains='RandomForest').count()
    xgb_detections  = ml_attack_qs.filter(ml_model_used__contains='XGBoost').count()
    lstm_detections = ml_attack_qs.filter(ml_model_used__contains='LSTM').count()
