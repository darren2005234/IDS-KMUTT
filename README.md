...
| Heartbleed | 0.9% | 2.6% | 0% | ✅ SID 30517 (added) | 0** | ❌ |

*\* FTP-Patator multiclass fails — RST saturation. Snort SID 2417 provides correct type.*  
*\*\* 0 BOTH on DDoS/Heartbleed: Snort and NFStream operate at different granularities; Heartbleed ML fails (8 flows + tcpreplay TLS limitation).*  
...
We apply SHAP (SHapley Additive exPlanations) to the XGBoost multiclass classifier to verify that decisions are driven by meaningful features rather than dataset artifacts.

**Global feature importance** (mean |SHAP|):

| Rank | Feature | SHAP | Category |
|---|---|---|---|
| 1 | `dst_port` | 0.955 | Port/Protocol |
| 2 | `src_port` | 0.470 | Port/Protocol |
| 3 | `src2dst_bytes` | 0.383 | Volume |
| 4 | `dst2src_stddev_ps` | 0.353 | Packet Size |
| 5 | `bidirectional_rst_packets` | 0.223 | TCP Flags |

**Per-class attack signatures** — each class has a distinct dominant feature:

| Attack | Top SHAP feature | Interpretation |
|---|---|---|
| PortScan | `src2dst_bytes` | Micro-flows (~40 B/SYN) |
| DDoS | `bidirectional_stddev_ps` | Packet size variance |
| SSH-Patator | `dst_port` | Port 22 near-exclusive |
| DoS | `bidirectional_mean_piat_ms` | Slowloris timing |
| Botnet | `src_port` | C&C beaconing regularity |

The SHAP analysis confirms that **DoS is detected through inter-packet timing** — a behavioral feature inaccessible to Snort's packet-level rules, explaining why DoS is ML_ONLY. See `notebooks/07_shap_explainability.ipynb` for the full analysis.
...
│   └── 07_shap_explainability.ipynb  # SHAP explainability analysis
│
...
| `07_shap_explainability.ipynb` | **SHAP analysis**: global + per-class feature importance, attack signatures, domain validation |
...
2. **Heartbleed**: Only 8 flows in CICIDS2017. Talos SID 30517 (TLSv1.2) added to `local.rules` (port 444) — Snort now detects the server-side leak as SNORT_ONLY. ML still fails (insufficient training flows + tcpreplay TLS limitation).
